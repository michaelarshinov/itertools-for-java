...
*/	
	...
	public static boolean is_sorted(int[]x) {
		int sorted[] = x.clone();
		Arrays.sort(sorted);
		return Arrays.equals(x, sorted);
		
	}
	public static String[] combinations(String[]w, int r) {
		int items=1;
		for (int i=w.length; i>r; i--) items*=i;
		for (int i=2; i<=(w.length-r);i++)		items/=i;
		
		String[] combinations=new String[items];
		Arrays.fill(combinations, " ");
